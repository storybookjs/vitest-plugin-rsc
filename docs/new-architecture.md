...
- Route-only `renderServer({ url })` coverage for `/notes`, `/notes/new`, `/notes/[id]`, and `/notes/[id]/edit` so Next resolves the page module, params, and loader tree itself.
...
13. Add route-only `renderServer({ url })` coverage for important existing notes demo pages that still render direct components with manual props, especially auth pages and profile.
