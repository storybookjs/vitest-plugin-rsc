...
- Version compatibility: supported stable Next, latest stable Next, canary Next, and optional-internal fallbacks for missing loaders such as `next-root-params-loader`. Explicit optimizer includes must be filtered through the installed Next package so a newer optional internal such as `next/dist/api/error` or `next/dist/client/components/catch-error` is not prebundled when older supported Next 16.x releases do not ship it. The browser App Router adapter also mirrors the dev websocket/static-indicator state that older Next 16.0/16.1 AppRouter builds expect, loads user `global-error` conventions into the app-render `ComponentMod` instead of leaving `GlobalError` undefined, and always passes a default `cacheLife` profile before project profiles so cache-components runtime setup is version-tolerant. Demo coverage for `next/error#unstable_catchError` must assert the real API when the installed Next version exports it and otherwise assert an explicit unsupported marker without crashing the page.
