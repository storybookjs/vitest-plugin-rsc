...
- Package coverage now proves the Vite adapter invokes Next's real `next-metadata-image-loader` for static metadata image files, including segment/basePath URL generation, content type, dimensions, and `.alt.txt` metadata. Notes-demo coverage proves metadata route exports for `robots`, `sitemap`, `manifest`, generated `opengraph-image`, and generated `twitter-image`. Still needed: static `icon`, `apple-icon`, and user-visible route execution for metadata routes instead of direct export invocation.
...
15. Add metadata route coverage for `generateImageMetadata`, `generateSitemaps`, static metadata files, `robots`, `sitemap`, `manifest`, `opengraph-image`, `twitter-image`, `icon`, `apple-icon`, and `favicon`. Done for the current loader-adapter layer: package coverage invokes Next's real metadata image loader for a static `opengraph-image` file with basePath, segment, dimensions, content type, and `.alt.txt` metadata. Done for direct metadata route exports: notes-demo covers `robots`, `sitemap`, `manifest`, generated `opengraph-image`, and generated `twitter-image`. Still needed: static `icon`/`apple-icon` route coverage, `generateSitemaps`, and user-visible route execution for metadata routes.
