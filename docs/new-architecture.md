...
- Version compatibility: supported stable Next, latest stable Next, canary Next, and optional-internal fallbacks for missing loaders such as `next-root-params-loader`. The browser App Router adapter also mirrors the dev websocket/static-indicator state that older Next 16.0/16.1 AppRouter builds expect, and always passes a default `cacheLife` profile before project profiles so cache-components runtime setup is version-tolerant.
