...
- Package coverage proves the Vite `"use cache"` hoist stays disabled when the loaded Next config does not set `cacheComponents: true`, and that `cacheHandlers`, `cacheMaxMemorySize`, project root, and dist dir are exposed to the app-render runtime from loaded Next config.

Missing before claiming support:

- Verify cached components with children and the encrypted `boundArgsLength` call shape used by Next's SWC transform. A local probe showed the current wrapper evaluates a cached component with distinct JSX children twice, so this needs the real Next transform call shape or an upstream Vite RSC hoist extension before it can be claimed.
...
- `next/cache`: cache-components with children, encrypted `boundArgsLength` call shape, deeper `fetch` cache semantics, and cache manifest/module mapping contracts. Custom cache handlers and configured cache memory size are wired from `next.config`; the transform-level disabled flag is covered: `"use cache"` directives are left untouched unless `cacheComponents: true` is loaded from Next config, and the current Vite hoist path is covered for closure-bound cached functions.
...
- Done for notes-demo basics: `cacheComponents: true`, cached async functions, custom `cacheHandlers`, configured `cacheMaxMemorySize`, `cacheLife`, `cacheTag`, default cache, remote cache, private cache request cookie access, closure-bound cached functions, public-cache `cookies()` errors, version-tolerant cold concurrent behavior, sequential duplicate cache hits, and cache invalidation through `updateTag`, `revalidateTag`, and `revalidatePath`.
...
- Still needed: cached components with children, encrypted `boundArgsLength` call-shape coverage, more dynamic API guard coverage, and cache manifest/module mapping coverage.
...
7. Done for the first cache-components slice: notes demo runs with `cacheComponents: true`, Vite RSC hoists async `use cache` functions, leaves `"use cache"` unhoisted when `cacheComponents` is disabled, covers closure-bound cached functions in the current Vite hoist path, wires custom `cacheHandlers` and `cacheMaxMemorySize`, and runtime goes through Next's `use-cache-wrapper` and cache handlers. Still needed: cached components with children, encrypted `boundArgsLength` call shape, more dynamic-API negative tests, and manifest mapping coverage.
