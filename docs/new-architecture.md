...
- Route-only `renderServer({ url })` coverage for `/notes` and `/notes/new` so Next resolves the page module and loader tree itself.
...
13. Add route-only `renderServer({ url })` coverage for important existing notes demo pages that still render direct components with manual props, especially `/notes/[id]`, auth pages, and profile.
