...
- Notes-demo coverage proves default `"use cache"` entries, `"use cache: remote"`, `"use cache: private"` request cookie access, `cacheTag`, a custom `cacheLife()` profile from `next.config`, closure-bound cached functions, and version-tolerant behavior for concurrent identical `"use cache"` calls. Current supported Next versions may either run two cold fills or coalesce them, but both concurrent call sites must agree with the final read count.
...
- `next/cache`: custom cache handlers, configured cache memory size, cache-components with children, encrypted `boundArgsLength` call shape, deeper `fetch` cache semantics, and cache manifest/module mapping contracts. The transform-level disabled flag is covered: `"use cache"` directives are left untouched unless `cacheComponents: true` is loaded from Next config, and the current Vite hoist path is covered for closure-bound cached functions.
...
- Still needed: cached components with children, encrypted `boundArgsLength` call-shape coverage, more dynamic API guard coverage, custom `cacheHandlers`, configured cache memory size, and cache manifest/module mapping coverage.
...
7. Done for the first cache-components slice: notes demo runs with `cacheComponents: true`, Vite RSC hoists async `use cache` functions, leaves `"use cache"` unhoisted when `cacheComponents` is disabled, covers closure-bound cached functions in the current Vite hoist path, and runtime goes through Next's `use-cache-wrapper` and cache handlers. Still needed: cached components with children, encrypted `boundArgsLength` call shape, custom handlers, more dynamic-API negative tests, and manifest mapping coverage.
