...
| 2. Routing data adapter                     | Done        | PR #47 / pending | `routing-data.test.ts`; rewrite ordering tests                          | Build-time routing conversion lives in the Vite plugin boundary; browser runtime consumes serialized routing data.            |
...
| 5. Optimizer entry architecture             | Done        | PR #47 / pending | optimizer entry tests; no-MSW app-shell regression                      | Broad `app/**` scan roots were replaced with a route-discovered virtual entrypoint warmup.                                    |
...
- 2026-05-16 Subgoal 2 support-matrix trial: added `@next/routing` as a
  ...
  - `pnpm --filter vitest-plugin-rsc test:run src/nextjs/request-router.test.ts`
  - `git diff --check`
  - CI: green on PR #47 for `d524c48`, including build, format, lint,
    typecheck, Vitest, preview, semantic title, and Next.js 16.2/latest/canary
    compatibility.
...
- 2026-05-16 Subgoal 2 review follow-up: fixed the latest reviewer feedback
  from the `d524c48..dc622d0` range before continuing further. Custom route
  destination conversion now consumes `:param*`, `:param+`, and `:param?`
  modifiers instead of leaving `*`, `+`, or `?` as literal query text. Request
  ...
  - `pnpm build`
  - `pnpm tsgo --build`
  - `pnpm test:run --project nextjs-notes-demo-browser --api 52647 'app/route-patterns/docs/[...slug]/page.test.tsx'`
  - `pnpm format:check`
  - `pnpm lint:ci`
  - `git diff --check`
  - CI: green on PR #47 for `0e6c469`, including build, format, lint,
    typecheck, Vitest, preview, semantic title, and Next.js 16.2/latest/canary
    compatibility.
- 2026-05-16 Subgoal 3 start: trying the real `AppPageRouteModule` path before
  committing to a broader app-page invoker split. Upstream inspection showed
  `AppPageRouteModule.render()` still delegates to `renderToHTMLOrFlight`, but
...
- 2026-05-16 Subgoal 2/3 review cleanup after `15e0567`: fetched the latest
  ...
  - `pnpm lint:ci`
  - `git diff --check`
  - CI: green on PR #47 for `f78b253`, including build, format, lint,
    typecheck, Vitest, preview, semantic title, and Next.js 16.2/latest/canary
    compatibility.
- 2026-05-16 Subgoal 4 start: evaluating the app-route invoker decision after
  fetching the latest PR review comments and confirming `f78b253` CI is green.
  Current code already makes `@next/routing` return `kind: "app-route"` for
  route handlers, and `testing-library.tsx` reports route-handler URLs as
  unsupported page render targets with a direct-import or future-helper hint.
  Decision: keep that explicit unsupported behavior for this PR instead of
  adding `AppRouteRouteModule.handle()`. Route-handler invocation through a real
  Next route module remains a future feature because implementing it now would
  broaden scope beyond the adapter cleanup, while the current tests still cover
  route handler primitives through direct imports.
  Local verification:
  - `pnpm --filter vitest-plugin-rsc test:run src/nextjs/request-router.test.ts`
...
- 2026-05-16 Subgoal 2 runtime-boundary cleanup: fetched the latest PR review
  ...
  - `git diff --check`
  - CI: pending push.
- 2026-05-16 Subgoal 5 start: fetched the latest PR comments before editing.
  The next slice replaces broad `app/**/*` and `src/app/**/*` optimizer scan
  roots with a route-discovered virtual entrypoint, or records a concrete
  blocker if Vite cannot warm optimizers from that virtual module.
- 2026-05-16 Subgoal 2 basePath review follow-up: fetched the latest PR
  comments and found a real routing-data bug before closing the optimizer
  slice. The plugin-side routing data must serialize `basePath`/`buildId`/i18n
  and normalize pathnames like Next's adapter output, while runtime request
  routing maps the resolved basePath-prefixed pathname back to raw discovered
  app route entries.
- 2026-05-16 Subgoal 2/5 completion: serialized `buildId`, `basePath`, and
  normalized i18n into `NextRoutingData`, normalized route pathnames with
  Next's adapter basePath behavior while keeping raw discovered route keys for
  `generateRoutesManifest()`, and mapped basePath-prefixed routing results back
  to raw discovered app routes in `request-router.ts`. Replaced broad
  `app/**/*` / `src/app/**/*` optimizer scan roots with
  `virtual:vitest-plugin-rsc/next-entrypoints`, generated from discovered
  route trees, their static route-module dependencies, and route handlers.
  Moved shared virtual ids to `virtual-ids.ts`. While validating the browser
