...
`"use cache"` is different from `"use client"` and `"use server"`. Next owns the cache semantics, request/cache stores, cache handlers, tags, and invalidation behavior, but this plugin must not hand the RSC graph to Next's RSC compiler to get there. The current supported path is a narrow Vite RSC hoist plus Next's cache runtime wrapper. Do not globally enable Next's RSC or Server Action transforms for `use cache`; add focused coverage and keep unsupported call shapes explicit instead.
