...
import { ArrowLeftIcon, NotebookPenIcon, PlusIcon } from "#components/icons.tsx";
...
export const noteFormSchema = zfd.formData({
...
export default async function NewNotePage() {
  await requireUser();
  const form = await getForm(noteFormSchema);
    ...
    <main className="relative mx-auto flex w-full max-w-2xl flex-1 flex-col px-4 pt-8 pb-16 sm:px-6 sm:pt-12 sm:pb-24">
      ...
      <form
        action={async (formData) => {
          ...
          const result = await setForm(noteFormSchema, formData);
          ...
          const [note] = await db
            .insert(notes)
            .values({ ownerId: user.id, title, content })
          ...
          <CardHeader className="gap-3 px-6 pt-7">
            <div className="flex items-center gap-3">
              ...
              <div className="space-y-0.5">
                ...
                <CardDescription>Give it a clear title; the body is optional.</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="px-6">
            <FieldGroup>
              ...
              <Field>
                ...
                <Textarea
                  key={form.old.content}
                  id="content"
                  name="content"
                  defaultValue={form.old.content}
