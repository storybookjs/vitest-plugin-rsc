...
import { cleanup, NextRouter, renderServer } from "vitest-plugin-rsc/nextjs/testing-library";
...
test("revalidatePath rerenders the current path without clearing explicitly tagged caches", async () => {
  await renderNextCacheProbe();
...
async function renderNextCacheProbe(label?: string) {
  ...
  await renderServer(
    <NextRouter url="/next-cache-probe">
      <NextCacheProbe />
    </NextRouter>,
  );
}
